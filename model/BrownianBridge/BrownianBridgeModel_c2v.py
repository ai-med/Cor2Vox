...
# code based on BBDM (https://arxiv.org/abs/2205.07680) #
#########################################################
...
class BrownianBridgeModel_c2v(nn.Module):
    ...
    def forward(self, x, y, condition=None, context=None):
        ...
        b, c, h, w, l, device, img_size, = *x.shape, x.device, self.image_size
        # assert h == img_size and w == img_size and l == img_size, f'height and width and depth of image must be {img_size}, but got {h} and {w} and {l}'
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(x, y, context=context, t=t)
    ...
    def p_losses(self, x0, y, context, t, noise=None):
        ...
        if self.loss_type == 'l1':
            ...
            recloss = (objective - objective_recon).pow(2).mean()
    ...
    @torch.no_grad()
    def p_sample_loop(self, y, context=None, condition=None, clip_denoised=True, sample_mid_step=False):
        if self.condition_key == "nocond":
            context = None
        else:
            context = y if context is None else context
    ...
    @torch.no_grad()
    def sample(self, y, context=None, condition=None, clip_denoised=True, sample_mid_step=False):
        return self.p_sample_loop(y, context=context, condition=condition, clip_denoised=clip_denoised, sample_mid_step=sample_mid_step)
