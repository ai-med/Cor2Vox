...
def resize_img(img, h_size, w_size, d_size):
    *_, h, w, d = img.shape
    ...
    if h != h_size or w != w_size or d != d_size:
        cop = tio.Resize((h_size, w_size, d_size))
        resized_img = []
        for i in range(img.shape[0]):
            resized_img.append(torch.from_numpy(cop(img[i].cpu().numpy())).unsqueeze(0).to(device))
        resized_img = torch.cat(resized_img, dim=0)
    return resized_img
...
class BaseRunner(ABC):
    def __init__(self, config):
        ...
        # set cls training params
        self.global_cls_epoch = 0  # global epoch
        self.global_cls_step = 0
                ...
                if (epoch + 1) % self.config.training.validation_interval == 0 or (
                    ...
                    if not self.config.training.use_DDP or \
                        ...
                        with torch.no_grad():
                            ...
                            if best_val_loss == -1.0 or average_loss < best_val_loss:
                                best_val_loss = average_loss
                                if self.config.args.save_best_val:
                                    print(f"saving best checkpoint on val at epoch {epoch}...")
                                    ...
                                    torch.save(model_states,
                                               os.path.join(self.config.result.ckpt_path, f'best_model.pth'))
                                    torch.save(optimizer_scheduler_states,
                                               os.path.join(self.config.result.ckpt_path, f'best_optim_sche.pth'))
    ...
    @torch.no_grad()
    def test(self):
        test_dataset = get_dataset(self.config.data, 'test')
        ...
        if self.config.data.dataset_type == 'c2v':
            sample_path = self.config.result.sample_to_eval_path
            sample_path = make_dir(os.path.join(sample_path, f'Test_samples_c2v'))
            ...
            for _, data in enumerate(test_loader):
                (x, x_name) = data[0]
                (x_cond, x_cond_name) = data[1]
                scan_id = data[-1]
                
                if len(data) > 3:
                    (condition, condition_name) = data[2]
                    test_batch = ((x, x_name), (x_cond, x_cond_name), (condition, condition_name))
                else:
                    test_batch = ((x, x_name), (x_cond, x_cond_name))
                ...
                real_output, fake_output = self.sample_results_c2v(self.net, test_batch)

                if self.config.data.dataset_config.to_normal:
                    real_output = real_output * 0.5 + 0.5
                    fake_output = fake_output * 0.5 + 0.5
                
                target_shape = (182, 218, 182) # target MRI shape
                real_output = resize_img(real_output, target_shape[0], target_shape[1], target_shape[2])
                fake_output = resize_img(fake_output, target_shape[0], target_shape[1], target_shape[2])
                ...
                if self.config.testing.save_syn_scans:                         
                    for batch_idx in range(whole_real_scan.shape[0]):
                        name = str(scan_id[batch_idx])
                        _whole_fake_scan = nib.Nifti1Image(whole_fake_scan[batch_idx].squeeze(), affine=affine)
                        _whole_fake_scan.to_filename(os.path.join(sample_path, f'{name}_syn_mri.nii.gz'))
                ...
                eva_data.append([str(scan_id[0]), MAE.item(), MSE.item(), PSNR.mean().item(), SSIM.item()])
            ...
            score_file = os.path.join(sample_path, '_scores.txt')
            ...
            df = pd.DataFrame(eva_data, columns=['ID', 'MAE', 'MSE', 'PSNR', 'SSIM'])

            mean_ssim = df['SSIM'].mean()
            median_ssim = df['SSIM'].median()
            ...
            print("+++++++++total number of samples = {} ++++++++".format(len(eva_data)))
            print(f"MAE: MEAN {mean_mae}, STD {std_mae}, Median {median_mae}, Max {max_mae}, Min {min_mae}")
            print(f"MSE: MEAN {mean_mse},STD {std_mse}, Median {median_mse}, Max {max_mse}, Min {min_mse}")
            print(f"PSNR: MEAN {mean_psnr}, STD {std_psnr}, Median {median_psnr}, Max {max_psnr}, Min {min_psnr}")
            print(f"SSIM: MEAN {mean_ssim}, STD {std_ssim}, Median {median_ssim}, Max {max_ssim}, Min {min_ssim}")
    
        else:
            raise NotImplementedError(f"Dataset type {self.config.data.dataset_type} is not supported for testing.")
